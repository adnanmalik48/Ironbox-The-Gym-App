...
<a href="{{ route('trainer_reviewer_reviews',request()->session()->get('trainer_reviewiers_id')) }}" class="btn btn-primary btn-sm float-right"  > Back</a>
</h4>

      <div class="card-body">
      <h5 class="card-title" >  <span style="font-weight:900;" class=" text-primary">   @foreach($trainerReviewers as $row)  @foreach ($row->user as $username)
                   {{$username->name }} 
