      ...
      <div class="col-md-12">
        ...
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">IRON BOX EXERCISE DETAILS
            <a href="{{ route('workout_plans_games',request()->session()->get('games_id')) }}" class="btn btn-primary btn-sm float-right"  > Back</a>
            </h4>
          ...
          <div class="table-responsive">
              <table id="datatable" class="table">
                ...
                <th style="font-size:15px;" >REPS </th>
                <th style="font-size:15px;" >DURATION</th>
                <th style="font-size:15px;" > ACTION</th>
                </thead>
                <tbody>
                @foreach($workoutPlanExercise as $row)

                  <tr>
                  <td>{{$row->id}}</td>
                  <td>{{$row->name}}</td>
                    <td>{{$row->reps}} reps per exercise</td>
                    <td> {{$row->duration}} weeks</td>
                  
                    
                    <td colspan="2">
                    <a href="{{ route('workout_plans_show_exercise',$row->id) }}"  class="btn btn-primary btn-sm " ><b>See Exercise Details</b></a>
