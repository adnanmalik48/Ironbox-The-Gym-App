      ...
      <div class="col-md-12">
        ...
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">IRON BOX GAME DETAILS
           
            <a href="{{ route('workout_plans_details',request()->session()->get('details_id')) }}" class="btn btn-primary btn-sm float-right"  >Back</a>
            </h4>
          ...
          <div class="table-responsive">
              <table id="datatable" class="table">
                ...
                <th style="font-size:15px;" > SETS </th>
                <th style="font-size:15px;" >Rounds</th>
                <th style="font-size:15px;" >ACTION</th>
                </thead>
                <tbody>
                @foreach($workoutPlanGames as $row)

                  <tr>
                  <td>{{$row->id}}</td>
                  <td>{{$row->name}}</td>
                    <td>{{$row->sets}} sets per game</td>
                    <td> {{$row->rounds}} rounds per game</td>
                  
                    
                    <td colspan="2">
                    <a href="{{ route('workout_plans_exercise',$row->id) }}"  class="btn btn-primary btn-sm " ><b>See Exercises</b></a>
